...
def build_master_db(main_folder: str, progress_callback: Callable[[str, int, int], None] | None = None) -> tuple[pd.DataFrame, list[dict]]:
    ...
    for frame in collected_frames:
        ...
        combined_rows.append(frame_copy[keep_cols])
...
with st.sidebar:
    ...
    input_mode = st.radio(
        "Source",
        options=["Local folder", "Upload ZIP", "Upload files"],
        help="Choose Local folder for desktop, Upload ZIP to upload a zipped folder, or Upload files for direct Excel uploads.",
    )
    main_folder = ""
    uploaded_files = []
    zip_file = None
    if input_mode == "Local folder":
        ...
        process_button = st.button("🚀 Process ZIP", use_container_width=True, type="primary")
    else:
        ...
        process_button = st.button("🚀 Process files", use_container_width=True, type="primary")
...
if process_button:
    if input_mode == "Local folder" and not main_folder:
        ...
        st.stop()
    if input_mode == "Upload ZIP" and zip_file is None:
